...
- Schedule slot cards show only start time, human status, and `↻` for existing recurring bookings or FREE slots where the booking-slots response returns `can_start_recurring: true`. Customer, phone, notes, price, and payment values remain outside the cards. Backend-authoritative `RECURRING_RESERVED` uses the ordinary `محجوز` presentation and loads its anchor Booking through `recurring_anchor_booking_id`; missing or failed anchors show a safe local error.
...
- Booking details use `إلغاء الحجز`, contain no backend-roadmap text, and show recurring context only as `↻ حجز أسبوعي` without a separate recurring product route. Strictly active recurrence adds secondary `إيقاف التكرار`; stopping preserves the current Booking, while cancellation and no-show warn that they also end recurrence. Active recurring reschedule remains hidden and unsupported.
- Optional `hold_expires_at` is integrated as the sole HOLD countdown source. Missing/invalid deadlines omit countdown copy; the frontend never derives a deadline from Court `internal_hold_expiry_hours` or booking creation time.
- Optional `recurrence_next` is integrated into recurring completion. Backend date/time, total price, required deposit, `can_continue`, and blocked reason drive the display; continuing sends only next-deposit method/reference/notes when required, while stopping sends `continue_recurring: false`. The frontend never calculates or sends the next deposit amount.
...
- Staff transaction requests ignore URL Court/collector overrides, stay assigned-Court scoped, and omit the employee selector. Management keeps named Court and collector filters.
- Settlement mutations require both effective membership permission and the preview capability (`can_approve` while not `is_self_preview`); a collector cannot approve or close their own custody.
...
- `/dashboard` is now the single operational Home labeled `الرئيسية`: real user greeting and Court/date context lead into today's accurate booking total, HOLD warning, aggregate `محتاجين إجراء`, and money custody. Existing analytics follow those operational sections; there is no parallel Home page.
...
- A visible `اسم العميل أو رقم الهاتف` field sends a debounced, URL-backed server `search` query and resets pagination without disturbing other filters.
